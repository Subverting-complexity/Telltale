...
export default function App() {
  ...
  // The process lists, on their own. Neither depends on how finely the chart
  // beside them is divided: /api/processes takes no bucket, and its answer for a
  // window cannot change when one is chosen. Both used to be fetched from the
  // same effect as the timeline, so every click on the detail picker re-ran the
  // two most expensive queries on the page to arrive back at the rows already on
  // screen.
  //
  // The two stay in one Promise.all, because they set the two halves of one
  // panel. Landing them separately would let Top Consumers be caught showing the
  // range's numbers under the Now heading, which is the confusion the toggle
  // exists to remove.
  useEffect(() => {
    setLoading(true);
